...
def syn_parser(workdir, target, setting):
    ...
    print(f"Generate cpp file for {target}")
    ...
    print(f"Compiling antlr4 cpp runtime for {target}")
    ...
    print("Done building antlr4-{target}!")


def main():
    parser = argparse.ArgumentParser(
        description="Create antlr4 parser for a given grammar in cpp that parses an input to tokens + parse_tree in json format"
    )
    parser.add_argument(
        "--workdir",
        type=str,
        default="_workdir",
        help="Path to the working directory for generated files (Default: _workdir)",
    )
